...
export function Header() {
  ...
  return (
    <header
      className="sticky top-0 py-2 z-[100] bg-[rgba(255,255,255,0.95)] backdrop-blur-[8px] border-b border-[#e2e8f0]"
    >
      <div
      ...
      >
        ...
        <nav className="flex items-center gap-1">
          ...
          {user ? (
            <Link
              to="/account"
              className="ml-2 flex hover:bg-[#1e3a5f]/90 items-center gap-2 bg-[#1e3a5f] text-white text-sm font-semibold px-3 py-2 rounded-lg no-underline"
            >
              {user.photoURL ? (
                <img width={30}
                  src={user.photoURL}
                  alt={firstName as any}
