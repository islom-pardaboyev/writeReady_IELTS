# Official IELTS Writing Band Descriptors (public version, updated May 2023)

Extracted from the IELTS.org PDF Islom supplied on 2026-09-20. This is the
yardstick for `test-essays.ts` — every `expected` band in that file is chosen by
matching the essay against the wording below, not by opinion.

`api/feedback.ts` -> `bandDescriptors()` is a condensed paraphrase of this same
document. It was checked against this text and is faithful, so the descriptors
are not the cause of any scoring drift; if you ever rewrite them, diff against
this file first.

Layout note: the PDF is a four-column table (Task Achievement/Response,
Coherence & Cohesion, Lexical Resource, Grammatical Range & Accuracy). Text
extraction flattens it, so each band number is followed by all four criteria in
that order.
...
Bolded text
indicates negative features that will limit a rating.
======================================================================
...
tendency to over-generalise
or there may be a lack of focus and
...
but rarely impede communication.
5
The main parts of the prompt are
...
Organisation
is evident but is not wholly logical
